/*******************************************************************
*
*  DESCRIPTION: class ParallelMCoordinator2
*
...
********************************************************************/
ParallelMCoordinator2::ParallelMCoordinator2( Coupled * coupled ) 
...
********************************************************************/
void ParallelMCoordinator2::initialize() 
{
...
********************************************************************/
ParallelProcessor &ParallelMCoordinator2::receive( const InitMessage &msg )
{

	//cout << "In function ParallelMCoordinator2::receive(InitMessage)"<<endl<<flush;
...
********************************************************************/
ParallelProcessor &ParallelMCoordinator2::receive( const CollectMessage &msg ) {
	...
	for( cursor = slaves().begin() ; cursor != slaves().end() ; cursor++ ) 
	{
		// Send to all slave coordinators
...
********************************************************************/
ParallelProcessor &ParallelMCoordinator2::receive( const InternalMessage &msg )
{
	...
	ParallelCoordinatorState::DependantList::const_iterator slavesCursor;

	//First, send message to ALL slave coordinators
	for( slavesCursor = slaves().begin() ; slavesCursor != slaves().end() ; slavesCursor++ ) 
	{
		doneCount( doneCount() + 1);
		send( internal , slavesCursor->first );
	}


	//Now, send to the imminent dependants.
...
********************************************************************/
ParallelCoordinator &ParallelMCoordinator2::sortExternalMessage( const BasicExternalMessage &msg )
{
	//cout << "In function ParallelMCoordinator2::receive(ExternalMessage)"<<endl << flush;
	...
	for( ; cursor != influList.end(); cursor++ ) {
		...
		//For messages received from parent processor, send to both, local childs
		//and slave coordinators

		//For messages received from slave coordinator, send only to 
		//local child.

		//If the destination model has a local master send message
		//to the local master, to the appropiate port
		//Otherwise, if the message came from the parent processor
		//find the slave processor that has to received the message
...
********************************************************************/
ParallelCoordinator &ParallelMCoordinator2::sortOutputMessage( const BasicOutputMessage &msg )
{

	//cout << "In function ParallelMCoordinator2::receive(OutputMessage)"<<endl << flush;

	bool isFromSlave;
	isFromSlave = (  slaves().find(msg.procId()) != slaves().end() );
	...
	for( ; cursor != influList.end(); cursor++ )
	{
		...
		// If not found in the output ports list, then it must be translated to an external message
...
********************************************************************/
ParallelProcessor &ParallelMCoordinator2::receive( const DoneMessage &msg )
{
...
********************************************************************/
VTime ParallelMCoordinator2::calculateNextChange(const VTime& time) const
{
...
********************************************************************/
BasicState* ParallelMCoordinator2::allocateState(){
