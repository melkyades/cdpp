...
#endif
  if(id == 0) {
    SequentialLP lp(34,34,1); // total, local, LPs
    ...
    MemSourceObject source24("source24", 4);
    source24.id = 24;
    source24.setDistribution(FIXED,25);
    source24.dest = 25;
    lp.registerObject(&source24);

    JoinObject join25("join25");
    join25.id = 25;
    join25.dest = 26;
    lp.registerObject(&join25);

    QueueObject queue26("queue26");
    queue26.id = 26;
    queue26.serverID = 27;
    lp.registerObject(&queue26);

    ForkObject fork27("fork27");
    fork27.id = 27;
    int outputs27[2] = { 28,30 };
    fork27.setForkDistribution(SPECIAL, 2, outputs27,0.85);
    lp.registerObject(&fork27);

    ServerObject server28("server28");
    server28.id = 28;
    server28.setServerDistribution(FIXED, 10);
    server28.dest = 29;
    lp.registerObject(&server28);

    JoinObject join29("join29");
    join29.id = 29;
    join29.dest = 24;
    lp.registerObject(&join29);
